...
class Json2csv:
    # コンストラクタ
    def __init__(self, json, keycsv):
        self.json = json
        self.keycsv = keycsv

    # Import JSON
    json_file = '../data/part_v003_o001_r_00000.json'
    def json2csv(self, json_file):
        '''
        :param json_file:
        :return: dfrt(dataframe includes RT), dfor(Dataframe witout RT)
        '''
        # Dates
        data = ijson.parse(open(self.json, 'r'), multiple_values=True)
        dates = [datetime.strptime(value, '%a %b %d %H:%M:%S +0000 %Y') for prefix, event, value in data if
                 prefix == 'created_at']
        ...
        df['RT_flag'] = df['tweet'].str.contains('RT') # RT_flag: True if tweet containt 'RT', otherwise False
    ...
    def keywords(self, filetype):
        ...
        dfw = pd.read_csv(self.keycsv, encoding='utf-8').rename(columns={'Unnamed: 0': 'types',
                                                                                          'file name': 'file_name',
